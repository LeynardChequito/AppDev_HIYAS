<template>
    <div>
      <q-table title="Event Student Attendance" :rows="tableData" :columns="columns" row-key="name" />
  ...
  onMounted(async () => {
    try {
      ...
      tableData.value = response.data.filter(student => student.event_type === 'Event');
    } catch (error) {
